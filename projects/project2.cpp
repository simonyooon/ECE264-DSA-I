...
// Unordered map that assigns a unique hash value for each last name on file
unordered_map<string, long long> lastNameMap = {
        {"ACOSTA", 1},
        {"ADAMS", 2},
        {"ADKINS", 3},
        {"AGUILAR", 4},
...
//Unordered map that assigns a unique hash value for each first name on file
unordered_map<string, long long> firstNameMap = {
  {"AALIYAH", 1},
	{"AARON", 2},
	{"ABEL", 3},
	{"ABIGAIL", 4},
...
// Simple structure that assigns a unique id and a corresponding pointer
struct People {
...
long long ssnToLong(string ssn) {
...
void radix_sort() {
        ...
        for(int i = 0; i < listSize; i++) {
                lastFive = ssnToInt(5, arrayPointers[i]->ssn);
                cup[lastFive][cupCount[lastFive]] = arrayPointers[i];
                cupCount[lastFive] += 1;
        }
        int a = 0;
        int b = 0;
        ...
        // for loop puts the pointer containing the SSN into cups
        for(int i = 0; i < listSize; i++) {
                firstFour = ssnToInt(4, arrayPointers[i]->ssn);
                cup[firstFour][cupCount[firstFour]] = arrayPointers[i];
                cupCount[firstFour] +=1;
        }
        a = 0;
        b = 0;
...
void insertion_sort() {
        ...
        for(int i = 1; i < listSize; i++) {
                if(arrayPointers[i]->lastName != arrayPointers[i - 1]->lastName
                  || arrayPointers[i]->firstName != arrayPointers[i - 1]->firstName) {
                        count = 0;
                }
                Data* key = arrayPointers[i];
                int j = i;
                while(j > (i - count) && ssnToInt(0, arrayPointers[j - 1]->ssn) > ssnToInt(0, key->ssn)) {
...
// Boolean to decide sort order
bool comparePeople(People a, People b) {
        return (a.id < b.id);
}

// Function that sorts the input list
void sortDataList(list<Data *> &l) {
        ...
        if(listSize < 200000) {
                ...
                for(int i = 0; i < listSize; i++) {
                        arrayNames[i].person = (*z++);
                        arrayNames[i].id = 1000000000000*lastNameMap[arrayPointers[i]->lastName] + 1000000000*firstNameMap[arrayPointers[i]->firstName] + ssnToLong(arrayPointers[i]->ssn);
                }
        ...
        // if statement to confirm correct array gets transferred
        if(test == 1 || test == 2) {
                for(int j = 0; j < listSize; j++) {
                        (*z++) = arrayNames[j].person;
                }
        }
        else {
                for(int j = 0; j < listSize; j++) {
